...
- **Option 1 (abandoned)**: Go to the detail page of your selected base station, and click ⌈Quick Data Download⌋. [**Note**: It seems that we can only download `.yyd` files from this quick link, so it cannot work in the *Emlid Studio*. Please refer to download **Option 2**. If you are a user for other PPK softwares, these files may still work as long as your software accept `.yyd` files (Hatanaka-compression RINEX) input]
<img src='/images/posts/ppk/download_option1.png'>

- **Option 2**: Download from [NCN Data and Product](https://geodesy.noaa.gov/CORS/data.shtml). You can select either server.
  - From NCN on AWS:
  ```bash
  ...
  # Check Supplementary (### More details about NCN data and products) for more information
  ```
  - From NGS servers:
  ```bash
...
The 4-char station ID `ssss` must be in lower cases.
{: .notice--warning }
...
### More details about NCN data and products
...
- Daily RINEX observation data can be found at:
  - NCN on AWS: https://noaa-cors-pds.s3.amazonaws.com/rinex/YYYY/DDD/ssss/File-Name
